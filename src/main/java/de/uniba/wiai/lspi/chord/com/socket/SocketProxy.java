...
public final class SocketProxy extends Proxy implements Runnable {
	...
	public Double getUptime() throws CommunicationException {
		this.makeSocketAvailable();

		logger.debug("Trying to get uptime.");

		/* prepare request for method getUptime */
		Request request = this.createRequest(MethodConstants.GET_UPTIME,
				new Serializable[0]);
		/* send request */
		try {
			logger.debug("Trying to send request " + request);
		...
		if (response.isFailureResponse()) {
			...
			try {
				Double result = (Double) response.getResult();
				return result;
			} catch (ClassCastException cce) {
				throw new CommunicationException(
						"Could not understand result! " + response.getResult());
